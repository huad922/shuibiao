...
*******************************************************************************/
/*uchar NB_CELL_RESELECTION(uchar time)
{
	uchar nb=0;
...
uchar NB_NUESTATS(uint8 time)
 {
	...
	while(CDMA_time)//等待接收时间
	{
		...
		if(Check_cmd(0,"RSRQ"))
		{
			...
			if(i==1)
			{
				g_write_value[51]=0x00;
				g_write_value[52]=RX0[a]-0x30;
			}
			else if(i==2)
			{
				C_DATA[51]=0x00;
